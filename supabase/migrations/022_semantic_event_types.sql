-- 022_semantic_event_types.sql
-- Improves rpc_update_entity: adds semantic event types for more fields,
-- and suppresses noisy internal fields from generating activity log entries.
--
-- Changes from 008_rich_activity_diffs.sql:
--   • assignee_id   → 'assignee_changed' (payload: {old, new} UUIDs)
--   • body          → 'body_changed'     (payload: omitted — too large)
--   • assignee_type → SKIPPED (always paired with assignee_id, never user-facing)
--   • sort_order    → SKIPPED (internal drag-ordering, not user-facing)
...
DECLARE
  v_allowed_tables text[] := ARRAY['tasks','meetings','library_items','diary_entries','grocery_items','companies','people','deals','essays'];
  v_protected_fields text[] := ARRAY['id','tenant_id','actor_id','created_at','ticket_id'];
  -- Fields that can be updated in the SET clause but must never generate an activity log entry
...
BEGIN
  ...
  -- Build SET clause (protected fields raise; activity_skip_fields are allowed in SET)
  ...
  FOR v_key IN SELECT jsonb_object_keys(p_fields) LOOP
    ...
    -- Skip internal-only fields (never user-facing)
    ...
    -- Build payload (omit body/notes content — can be very large)
