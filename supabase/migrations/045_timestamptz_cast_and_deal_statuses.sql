-- 045_timestamptz_cast_and_deal_statuses.sql
-- 1) Fixes rpc_update_entity: adds ::timestamptz cast for last_enriched,
--    ::uuid for primary_contact_id, ::date for expected_close_date.
--    Also enriches assignee_changed payload with new_label display name.
-- 2) Expands deals status CHECK constraint with pipeline stages.

-- ---------------------------------------------------------------------------
-- 1. rpc_update_entity — timestamptz cast + assignee label enrichment
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  FOR v_key IN SELECT jsonb_object_keys(p_fields) LOOP
    ...
    ELSIF v_key IN ('last_enriched') THEN
...
-- ---------------------------------------------------------------------------
-- 2. Expand deals status CHECK — add qualified, proposal, negotiation
-- ---------------------------------------------------------------------------

ALTER TABLE deals DROP CONSTRAINT IF EXISTS deals_status_check;
ALTER TABLE deals ADD CONSTRAINT deals_status_check
  CHECK (status IN ('prospect', 'qualified', 'proposal', 'negotiation', 'active', 'won', 'lost'));
