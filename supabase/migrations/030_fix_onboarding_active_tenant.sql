-- Fix: rpc_setup_workspace was not setting active_tenant_id on the profile,
-- causing requireAdmin() to fail (tenant_role always null).
...
BEGIN
  ...
  UPDATE profiles SET active_tenant_id = v_tenant_id WHERE id = v_user_id;
